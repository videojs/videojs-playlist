...
QUnit.test('includes the video description if provided', function(assert) {
  player.playlist(playlist);
  player.playlistUi();

  const items = document.querySelectorAll('.vjs-playlist-item');

  assert.equal(items[0].querySelector('.vjs-playlist-description').textContent,
        playlist[0].description,
        'wrote the name');
  assert.equal(items[1].querySelector('.vjs-playlist-description'),
        null,
