...
class PlaylistMenu extends Component {
  ...
  createPlaylist_() {
    ...
    // playback.
    if (!this.el_.querySelector('.vjs-playlist-ad-overlay')) {
    ...
    let showHide = this.el_.querySelector('.vjs-playlist-show-hide');
    if (!showHide) {
      showHide = new PlaylistShowHideButton(this.player_, this);
      this.addChild(showHide);
    } else {
      // Move to bottom so it's visible
      this.el_.appendChild(showHide);
    }
...
class PlaylistShowHideButton extends Component {

  constructor(player, playlist) {
    super(player);
    this.hidden_ = false;
    this.playlist_ = playlist;
    this.on(['click', 'tap'], this.showHide_);
  }

  showHide_() {
    if (this.hidden_) {
      this.playlist_.removeClass('vjs-playlist-hidden');
      this.player_.removeClass('vjs-playlist-hidden');
    } else {
      this.playlist_.addClass('vjs-playlist-hidden');
      this.player_.addClass('vjs-playlist-hidden');
    }
    this.hidden_ = !this.hidden_;
    this.setText_(this.el_);
  }

  setText_(el) {
    if (this.hidden_) {
      let text = this.localize('Show Playlist');
      el.textContent = '';
      el.appendChild(document.createTextNode(text));
      el.setAttribute('title', text);
    } else {
      let text = this.localize('Hide Playlist');
      el.textContent = '';
      el.appendChild(document.createTextNode(text));
      el.setAttribute('title', text);
    }
  ...
  createEl() {
    ...
    el.className = 'vjs-playlist-show-hide';
...
videojs.registerComponent('PlaylistShowHideButton', PlaylistShowHideButton);
